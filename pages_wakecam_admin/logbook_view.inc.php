...
<div class="main">
	<div class="main_text">
	<div class="main_text_title">View Logbook</div>
  ...
  ?>
    ...
    // Zeiteinschränken ;-)
    $sql = "SELECT B.payDriver, B.riderID, A.rideID,A.start,B.priceTotal,C.name as boatName, B.riderName as NonMemberRiderName, CONCAT(D.first_name ,' ', D.last_name) as riderName, D.campRider, CONCAT(E.first_name ,' ', E.last_name) as driverName, F.name AS sportName FROM rideTimes AS A LEFT JOIN rides AS B ON A.rideID = B.ID LEFT JOIN boats AS C ON B.boatID=C.ID LEFT JOIN members AS D ON B.riderID = D.ID  LEFT JOIN members AS E ON B.driverID = E.ID LEFT JOIN sports AS F ON B.sportID=F.ID";
    ...
    $sql = "SELECT A.ID,UNIX_TIMESTAMP(A.ts) as start,A.loginID,B.name as boatName,CONCAT(C.first_name, ' ', last_name) AS driverName,A.price FROM maintenance AS A LEFT JOIN boats AS B ON A.boatID = B.ID LEFT JOIN members AS C ON A.driverID = C.ID WHERE A.oil IS NULL AND A.filter IS NULL AND A.rideID IS NULL AND A.price > 0";
    ...
    ?>
    <div class="main_text_title">Logbook for  <?php echo $MonthArr[$_SESSION["view_logbook"]["m"]] . " " . $_SESSION["view_logbook"]["Y"];?></div><br />
    ...
    if (isset($rideData) && is_array($rideData)){
		...
		foreach ($rideData as $time=>$rideData2){
	    	...
	    	echo "<br /><div class=\"main_text_title\">Logbook for " . $match[3] . "." . $match[2] . "." . $match[1] . "</div>";
	    	?>
            
            <div class="hours_in_view_logbook">
   			<?php
			   $boats = getAllBoats();
			   foreach ($boats as $boat){
				   $sql = "SELECT engineTime FROM maintenance WHERE boatID = '" . $boat["ID"] . "' AND ts like '" . $match[1] . "-" . $match[2] . "-" . $match[3] . "%' LIMIT 0,1";
				   $sql2 = "SELECT engineTime FROM maintenance WHERE boatID = '" . $boat["ID"] . "' AND ts > '" . $match[1] . "-" . $match[2] . "-" . $match[3] . " 23:59:59' LIMIT 0,1";
				   //echo $sql."<br><br>";
				   //echo $sql2."<br><br>";
				   ...
				   if (isset($enginehours) && isset($enginehours2) && $enginehours != "-1" && $enginehours2 != "-1"){
					   $enginesecs = round(abs(($enginehours - $enginehours2)),2);
						...
						$sql = "SELECT sum(stop-start) as time FROM rideTimes AS A
							LEFT JOIN rides AS B ON A.rideID = B.ID
							WHERE A.start > '" . mktime(0,0,0,$match[2],$match[3],$match[1]) . "' AND A.start < '" . mktime(23,59,59,$match[2],$match[3],$match[1]) . "'
	      ...
	      <tr>
	        <td width="75">Time</td>
	        <td width="175">Boat</td>
	        <td width="250">Driver</td>
	        <td width="130">Type</td>
	        <td width="250">Rider Name</td>
	        <td width="50"><strong>In</strong></td>
	        <td width="50"><strong>Out</strong></td>
	      </tr>
	      <?php
	      	$i = 0;
                $totalIn = 0;
