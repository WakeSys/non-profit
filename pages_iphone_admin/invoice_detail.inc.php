...
if (
	isset($_POST["nights"]) && 
	is_numeric($_POST["nights"]) && 
	$_POST["nights"] > 0 &&
	isset($_POST["nightprice"]) && 
	is_numeric($_POST["nightprice"]) && 
	$_POST["nightprice"] > 0
)
{
	if (
		!isset($_POST["creditID"]) || 
		!is_numeric($_POST["creditID"])
	)
	{
		//print_r($_POST);
		$sql = "
	...
	else 
	{
		$sql = "
			UPDATE 
				credits 
			SET 
				campNights = '" . $_POST["nights"] . "',
				value = '" . ($_POST["nights"] * $_POST["nightprice"]) . "' 
			WHERE 1
			AND ID = '" . $_POST["creditID"] . "'
		";
...
)
{
	if (!is_numeric($_POST["add"]))
	{
		...
		if(isset($_POST["pass"]))
		{
			...
			if (
				md5($_POST["pass"]) == $DB_PASS || 
				$_POST["pass"] == STANDARD_PASS
			)
			{
			...
			else 
			{
				...
				if(
					isset($_SESSION["user"]["error"]) && 
					$_SESSION["user"]["error"] > 3
				)
				{
					$logout = "true";	
				}
...
)
{
	...
	echo '<form action="https://www.paypal.com/uk/cgi-bin/webscr" method="post" id="PayPalForm">';
		echo '<h2>Pay with Paypal</h2>';
		echo '<ul>';
			echo "--TITLE--Paypal--";
		...
		echo '</ul>';
		echo '<h3>In order to mark the invoice as paid you have to click on "Return to Shop", on the PayPal site, after the payment has been carried out</h3>';
	    
		echo '<input type="hidden" name="cmd" value="_xclick" />';
		echo '<input type="hidden" name="business" value="' . $preferences[0]["PaypalMail"]. '" />';
		echo '<input type="hidden" name="item_name" value="' . $preferences[0]["contact_name_of_school"]. '" />';
		echo '<input type="hidden" name="cancel_return" value="http://'. $_SERVER["HTTP_HOST"] . '/index.php?paypal=error';
		foreach ($_POST as $key => $elem)
		{
			if ($key != "pass")
			{
		...
		echo '" />';
	   	echo '<input type="hidden" name="return" value="http://' . $_SERVER["HTTP_HOST"] . '/index.php?paypal=true';
		foreach ($_POST as $key => $elem)
		{
			if ($key != "pass")
			{
		...
		echo '" />';
		echo '<input type="hidden" name="currency_code" value="' . getPreferences("currencyShort") .'" />';
		echo '<input type="hidden" name="amount" value="' . $_POST["add"] . '" />';
		echo '<input type="hidden" name="notify_url" value="http://chris.wakesystem.werbungmueller.de/paypal.php" />';
	echo '</form>';
...
)
{
	...
	{
            if (!isset($_POST["invoice_mail"]))
        	{
        		$_POST["invoice_mail"] = null;
        	}
...
)
{
	...
	elseif (
		isset($_POST["invoice_detail"]) && 
		(
			$_POST["invoice_detail"] == "member_today" || 
			$_POST["invoice_detail"] == "nonMember_all" || 
			$_POST["invoice_detail"] == "nonMember_today"
		) 
	)
	{
		echo "--BACK_PAGE--invoice_detail--";
	...
	if (
		isset($memberData[0]["campRider"]) && 
		$memberData[0]["campRider"] == "yes"
	)
	{
	...
	echo '<h2>Safety</h2>';
	echo '<ul>';
		echo '<li>Driver Password';
			echo '<span class="showArrow secondaryWArrow">';
				echo '<label>';
					echo '<input class="secondaryInput" name="pass" type="password" size="12" maxlength="30" />';
				echo '</label>';
			echo '</span>';
		echo '</li>';
	echo '</ul>';
    ...
    if ( isset($_POST["rideID"]) && is_numeric($_POST["rideID"]))
    {
        ...
        echo '<ul>';
			echo '<li>Email';
				echo '<span class="showArrow secondaryWArrow">';
					echo '<label>';
						echo '<input class="secondaryInput" name="invoice_mail" type="email" size="12" maxlength="100" />';
					echo '</label>';
				echo '</span>';
			echo '</li>';
        echo '</ul>';
...
else 
{
	...
	)
	{
		...
		$sql = "
			...
			WHERE 1
			AND A.invoiceID IS NULL AND B.campRider = 'yes' 
			GROUP BY A.memberID
		";
	...
	)
	{
		...
		$sql = "
			...
			WHERE 1
			AND A.invoiceID IS NULL 
			AND (A.memberID IS NULL OR A.memberID = 0) 
			AND A.rideID IN (
				SELECT 
					rideID 
				FROM 
					rideTimes 
				WHERE 1
				AND start BETWEEN '" . mktime(0,0,0,date("m"),date("d"),date("Y")) . "' AND '" . mktime(23,59,59,date("m"),date("d"),date("Y")) ."' 
				GROUP BY rideID
			)
		";
		//echo $sql;
